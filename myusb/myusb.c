...
static void panicb_int_callback(struct urb *urb) {
...
static long panicb_ioctl(struct file *file, unsigned int cmd,
                         unsigned long arg) {
  struct usb_panicb *dev;
  unsigned int button;
  unsigned long flags;
...
static int panicb_probe(struct usb_interface *interface,
  ...
  for (i = 0; i < iface_desc->desc.bNumEndpoints; i++) {
    ...
    pr_info(
        ...
        ep->bEndpointAddress, ep->bmAttributes, usb_endpoint_maxp(ep),
        ep->bInterval);
  ...
  usb_fill_int_urb(panicb_dev->int_in_urb, panicb_dev->udev,
                   usb_rcvintpipe(panicb_dev->udev, panicb_dev->int_in_ep),
                   panicb_dev->int_in_buffer, panicb_dev->int_in_size,
