#ifndef _CHARIOCTL_H_
#define _CHARIOCTL_H_
...
/*
 * charioctl.h - the header file with the ioctl definitions.
 *
 * The declarations here have to be in a header file, because they need to be known both to the kernel module (in charioctl.c) and the process calling ioctl() (in userspace_ioctl.c).
 */
...
/* The major device number. We can not rely on dynamic registration any more, because ioctls need to know it. */
#define MAJOR_NUM 100

/* Set the mesage of the device driver */
 ...
 * However, we still need the buffer to place the message in to be input, as it is allocated by the process.
 */
